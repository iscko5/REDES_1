...
void crearTrama (unsigned char *trama){             //TRAMA PARA PROTOCOLO ARP
    //ENCABEZADO MAC
...
void recibirTrama (int ds, unsigned char * trama){
  ...
  for (int i = 0; i < 4; i++) {     //IMPRESION DE LA IP SIN IMPORTAR EL RESULTADO
     printf("%d.", IPDestino[i]);
  }
  printf("  --------------  ");

  while (mtime < 200) {         //LIMITE DE TIEMPI EN MLS PARA RECIBIR UNA RESPUESTA 
    ...
    if(tam == -1){
      ...
      if (!memcmp(trama + 0, MACOrigen, 6) || !memcmp(trama + 6, MACOrigen, 6)) {
        ...
        if (!memcmp(trama + 12, EthertypeARP, 2) ){        //COMPROBAMOS PROTOCOLO ARP
          //ImpTrama(trama, tam);
...
void ImpTrama(unsigned char * trama, int len){        //FUNCION PARA IMPRIMIR LA TRAMA
  int i;
...
void ImpMac(unsigned char * trama){           //FUNCION PARA IMPRIMIR LA MAC DESDE LA TRATA RECIBIDA
